...
namespace Subs4.Presentation
{
    ...
    {
        //private readonly SubscriptionDoc3 gSubscriptionDoc;
        //private readonly SubscriptionDoc3.SubscriptionDataTable gSubscriptionTable;
        ...
        {
            InitializeComponent();
            //gSubscriptionDoc = (SubscriptionDoc3)this.Resources["subscriptionDoc3"];
            //gSubscriptionTable = gSubscriptionDoc.Subscription;
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            //Subs.Data.SubscriptionDoc3 subscriptionDoc3 = ((Subs.Data.SubscriptionDoc3)(this.FindResource("subscriptionDoc3")));
        ...
        private void ButtonGenerate_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("You did not select something that I can generate an invoice from.");
                    return;
                }
                ...
                foreach (SubscriptionData3 lSelectedSubscription in gSelectedSubscriptions.View)
                {
                    ...
                    if (lSelectedSubscription.ProFormaId == null)
                    {
                        ...
                        if (lProFormaId != null)
                        {
                            MessageBox.Show("TheProforma Invoice Numbers of your list should all be null or all have the same value.");
                            return;
                        }
                    }
                    else
                    {
                        ...
                        {
                            MessageBox.Show("The ProFormaId your list should all have the same pro-forma id.");
                            return;
                        }
                    }
                ...
                if (!OneInvoice(lPayerId, (int)lProFormaId))
                {
                    return;
                }
        ...
        private bool SendEmailInvoice(string FileName, string Destination, int PayerId)
        {
            try
            {
                ...
                myMessage.Subject = "SUBS Proforma Invoice - Customerid = " + PayerId.ToString();
 ...
 + "www.mims.co.za";
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, this.ToString(), "SendEmailInvoice", "");
                    CurrentException = CurrentException.InnerException;
                } while (CurrentException != null);
        ...
        private bool OneInvoice(int PayerId, int pInvoiceId)
        {
            ...
            try
            {
                ...
                // Create one invoice for the previous batch
...

                {
                    ...
                    if ((lResult = LedgerData.GetReportInfo(PayerId, ref lLedgerDoc)) != "OK")
                    {
                        MessageBox.Show(lResult);
                        return false;
                    }
                ...
                lLedgerDoc.ReportInfo[0].InvoiceId = pInvoiceId;
                ...
                if (!lLedgerDoc.ReportInfo[0].IsInvoiceIdNull()) lExcelIO.PutCellString(lStage, 1, 1, "PRO" + lLedgerDoc.ReportInfo[0].InvoiceId.ToString());

                lStage = "PersonalData";
                if (!lLedgerDoc.ReportInfo[0].IsPPhoneNumberNull()) lExcelIO.PutCellString(lStage, 1, 1, lLedgerDoc.ReportInfo[0].PPhoneNumber);
                if (!lLedgerDoc.ReportInfo[0].IsPEmailNull()) lExcelIO.PutCellString(lStage, 2, 1, lLedgerDoc.ReportInfo[0].PEmail);
                //if (!lLedgerDoc.ReportInfo[0].IsPVATRegistrationNull()) lExcelIO.PutCellString(lStage, 3, 1, lLedgerDoc.ReportInfo[0].PVATRegistration);
                ...
                lExcelIO.PutCellString(lStage, 7, 1, lLedgerDoc.ReportInfo[0].PayerId.ToString());

                lStage = "Date";
                lExcelIO.PutCellDate(lStage, 1, 1, DateTime.Now);

                lStage = "BillingAddress";
                if (!lLedgerDoc.ReportInfo[0].IsLine1Null()) lExcelIO.PutCellString(lStage, 1, 1, lLedgerDoc.ReportInfo[0].Line1);
                if (!lLedgerDoc.ReportInfo[0].IsLine2Null()) lExcelIO.PutCellString(lStage, 2, 1, lLedgerDoc.ReportInfo[0].Line2);
                if (!lLedgerDoc.ReportInfo[0].IsLine3Null()) lExcelIO.PutCellString(lStage, 3, 1, lLedgerDoc.ReportInfo[0].Line3);
                if (!lLedgerDoc.ReportInfo[0].IsLine4Null()) lExcelIO.PutCellString(lStage, 4, 1, lLedgerDoc.ReportInfo[0].Line4);
                if (!lLedgerDoc.ReportInfo[0].IsLine5Null()) lExcelIO.PutCellString(lStage, 5, 1, lLedgerDoc.ReportInfo[0].Line5);
                if (!lLedgerDoc.ReportInfo[0].IsLine6Null()) lExcelIO.PutCellString(lStage, 6, 1, lLedgerDoc.ReportInfo[0].Line6);
                if (!lLedgerDoc.ReportInfo[0].IsLine7Null()) lExcelIO.PutCellString(lStage, 7, 1, lLedgerDoc.ReportInfo[0].Line7);
                if (!lLedgerDoc.ReportInfo[0].IsLine8Null()) lExcelIO.PutCellString(lStage, 8, 1, lLedgerDoc.ReportInfo[0].Line8);

                lStage = "PhysicalAddress";
                if (!lLedgerDoc.ReportInfo[0].IsPAddress1Null()) lExcelIO.PutCellString(lStage, 1, 1, lLedgerDoc.ReportInfo[0].PAddress1);
                if (!lLedgerDoc.ReportInfo[0].IsPAddress2Null()) lExcelIO.PutCellString(lStage, 2, 1, lLedgerDoc.ReportInfo[0].PAddress2);
                if (!lLedgerDoc.ReportInfo[0].IsPAddress3Null()) lExcelIO.PutCellString(lStage, 3, 1, lLedgerDoc.ReportInfo[0].PAddress3);
                if (!lLedgerDoc.ReportInfo[0].IsPAddress4Null()) lExcelIO.PutCellString(lStage, 4, 1, lLedgerDoc.ReportInfo[0].PAddress4);
                if (!lLedgerDoc.ReportInfo[0].IsPAddress5Null()) lExcelIO.PutCellString(lStage, 5, 1, lLedgerDoc.ReportInfo[0].PAddress5);
                if (!lLedgerDoc.ReportInfo[0].IsPAddress6Null()) lExcelIO.PutCellString(lStage, 6, 1, lLedgerDoc.ReportInfo[0].PAddress6);
                if (!lLedgerDoc.ReportInfo[0].IsPAddress7Null()) lExcelIO.PutCellString(lStage, 7, 1, lLedgerDoc.ReportInfo[0].PAddress7);
                if (!lLedgerDoc.ReportInfo[0].IsPAddress8Null()) lExcelIO.PutCellString(lStage, 8, 1, lLedgerDoc.ReportInfo[0].PAddress8);
                ...
                //foreach (DataRowView lSelectedRowView in gSelectedSubscriptions.View)
                //{
                //    SubscriptionDoc3.SubscriptionRow lSelectedRow = (SubscriptionDoc3.SubscriptionRow)lSelectedRowView.Row;

                //    lExcelIO.PutCellString("Items", i + 1, 1, lSelectedRow.SubscriptionId.ToString());
                //    lExcelIO.PutCellString("Items", i + 1, 2, lSelectedRow.ReceiverSurname);
                //    lExcelIO.PutCellString("Items", i + 1, 3, lSelectedRow.StartIssueDescription);
                //    lExcelIO.PutCellString("Items", i + 1, 4, lSelectedRow.LastIssueDescription);
                //    lExcelIO.PutCellDecimal("Items", i + 1, 5, lSelectedRow.BaseRate);  // Full price
                //    lExcelIO.PutCellDecimal("Items", i + 1, 6, lSelectedRow.BaseRate * (1 - lSelectedRow.DiscountMultiplier));
                //    lExcelIO.PutCellDecimal("Items", i + 1, 7, lSelectedRow.DeliveryCost);
                //    lExcelIO.PutCellDecimal("Items", i + 1, 8, lSelectedRow.Vat);
                //    lExcelIO.PutCellDecimal("Items", i + 1, 9, lSelectedRow.UnitPrice);
                //    lExcelIO.PutCellString("Items", i + 1, 10, lSelectedRow.UnitsPerIssue.ToString() + " X " + lSelectedRow.NumberOfIssues.ToString());
                //    lExcelIO.PutCellDecimal("Items", i + 1, 11, lSelectedRow.UnitPrice * lSelectedRow.UnitsPerIssue * lSelectedRow.NumberOfIssues);
