...
namespace Subs4.Presentation
{
    ...
    public partial class Maintenance : Window
    {
        ...
        private string RenderStatement(int pCustomerId, int pStatementId)
        {
            try
            {
                StatementControl2 lStatementControl = new StatementControl2();
                return lStatementControl.Load(pCustomerId, pStatementId); ;
            }
        ...
        public static bool SendEmailInvoice(string FileName, string Destination, int PayerId)
        {
            try
            {
                ...
                myMessage.Subject = "SUBS Tax Invoice - Customerid = " + PayerId.ToString();
            ...
            {
                MessageBox.Show("Error in Maintenance.SendEmailInvoice" + Ex.Message.ToString());
                // Record the event in the Exception table
                ExceptionData.WriteException(1, Ex.Message, "Mainenance", "SendEmailInvoice", FileName.ToString());
                return false;
            }
        }
        ...
        private void buttonInvoiceDirective_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                LedgerData.LoadInvoiceBatch(int.Parse(textFromTransactionId.Text), ref gLedgerDoc);
                ...
                foreach (LedgerDoc2.InvoiceBatchRow lRow in gLedgerDoc.InvoiceBatch)
                {
                    ...
                    if (lRow.PayerId != lCurrentPayerId)
                    {
                        ...
                        //lCurrentInvoiceDescription = new InvoiceDescription() { PayerId = lRow.PayerId, InvoiceId = lRow.InvoiceId, EMailAddress = lRow.EMailAddress };
                    }
                    ...
                    // Add the subscription to the binary directive
                    //lCurrentInvoiceDescription.Subscriptions.Add(new InvoiceSubscriptionPart() { SubscriptionId = lRow.SubscriptionId, TransactionId = lRow.TransactionId });
                ...
                if (lCurrentPayerId != 0)
                {
                    ...
                    // Save a binary copy of previous invoice directive
                    //lBinaryFormatter.Serialize(lMemoryStream, lCurrentInvoiceDescription);
                    ...
                    //LedgerData.InvoiceDirective(lCurrentInvoiceId, lByteArray);
        ...
        //private void buttonInvoiceDirectiveForSubscription_Click(object sender, RoutedEventArgs e)
        //{
        //    try
        //    {
        //        // See if the previous run is cleaned up.
        ...
        //        LedgerData.LoadInvoiceForSubscription(int.Parse(textForSubscriptionId.Text), ref gLedgerDoc);
        ...
        //        {
        //            ExceptionLevel++;
        //            ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, this.ToString(), "buttonInvoiceDirectiveForSubscription_Click", "");
        //            CurrentException = CurrentException.InnerException;
        //        } while (CurrentException != null);

        //        MessageBox.Show("Error in buttonInvoiceDirectiveForSubscription_Click " + ex.Message);
        //    }
        //    finally
        //    {
        //        this.Cursor = Cursors.Arrow;
        ...
        private void buttonInvoiceEmail_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                if (!Directory.Exists(Settings.DirectoryPath + "\\Fax") | !Directory.Exists(Settings.DirectoryPath + "\\Post") | !File.Exists(Settings.DirectoryPath + "\\InvoiceBatchDone.xml"))
                {
                ...
                foreach (LedgerDoc2.InvoiceBatchRow lRow in gLedgerDoc.InvoiceBatch)
                {
                    ...
                    if (CurrentCustomerId != lRow.PayerId)
                    {
                        ...
                        string FilePrefix = "\\INV_" + lRow.PayerId.ToString() + "_" + lRow.InvoiceId.ToString();

                        if (!lRow.IsEMailAddressNull())
                        {
                            if (!SendEmailInvoice(Settings.DirectoryPath + FilePrefix + ".pdf", lRow.EMailAddress, lRow.PayerId))
                            {
                                return;
                            }
                ...
                ExceptionData.WriteException(5,
                    ...
                    this.ToString(), "btnEmailInvoice", CurrentCustomerId.ToString());
            }
        ...
        private void buttonInvoiceSelectSkipIssue_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                IssuePicker2 lIssuePicker = new Subs4.Presentation.IssuePicker2();
