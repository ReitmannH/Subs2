...
namespace Subs.Business
{
    public static class CustomerBiz
    {
        ...
        private static readonly Subs.Data.LedgerDoc2TableAdapters.TransactionsTableAdapter gTransactionAdapter = new Subs.Data.LedgerDoc2TableAdapters.TransactionsTableAdapter();
        ...
        static CustomerBiz()
        {
            gTransactionAdapter.AttachConnection();
        }
        #endregion
        ...
        public static string SynchronizeLiability(CustomerData3 pCustomerData, [CallerMemberName] string pCaller = null)
        {
            try
            {
...

                {
                    ...
                    if ((lResult = CustomerData3.CalulateLiability(pCustomerData.CustomerId, ref lLiabilityRecords, ref lCalculatedLiability)) != "OK")
                    {
        ...
        public static string Pay(ref PaymentData.PaymentRecord pRecord, out int pTransactionId)
        {
            ...
            if (LedgerData.DuplicatePayment(pRecord.CustomerId, pRecord.Reference, pRecord.Amount) > (int)0)
            {
        ...
        public static string ReverseWriteOffMoney(int pTransactionId, int pInvoiceId, int pPayerId, decimal pAmount, string pExplanation)
        {
            ...
            int lNumberOfReversals = (int)gTransactionAdapter.CheckReference(pExplanation, (int)Operation.ReverseWriteOffMoney, pPayerId);
        ...
        public static string ValidateDuplicate(CustomerData3 pCustomerData)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(pCustomerData.PhoneNumber)) return "Phone is compulsory";
        ...
        public static string Cancel(ref Subs.Data.CustomerData3 pCustomerData, string pReference)
        {
            SqlConnection lConnection = new SqlConnection();

            try
            {
                // Verify that one can cancel this customer

                if (pCustomerData.Status == CustomerStatus.Cancelled)
                {
                    return "You cannot cancel a cancelled customer!";
                }
                ...
                //bool Dependent = true;
                //if (!pCustomerData.CheckSubscriptionDependency(ref Dependent))
                //{
                //    return "Error in checking subscription dependency";
                //}
                ...
                SqlTransaction lTransaction;
                lConnection.Open();
                lTransaction = lConnection.BeginTransaction("CancelCustomer");

                // Update the Customer

                pCustomerData.Status = CustomerStatus.Cancelled;

                {
                    string lResult;

                    if ((lResult = pCustomerData.UpdateInTransaction(ref lTransaction)) != "OK")
                    {
                        lTransaction.Rollback("CancelCustomer");
                        return lResult;
                    }
                }


                //if (!pCustomerData.Cancel(ref myTransaction, pCustomerData.CustomerId))
                //{

                //    return "Error in canceling customer";
                //}

                // Update the ledger

                LedgerData.CancelCustomer(ref lTransaction, pCustomerData.CustomerId, pReference);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, "CustomerBizStatic", "Cancel", "");
                    CurrentException = CurrentException.InnerException;
                } while (CurrentException != null);
