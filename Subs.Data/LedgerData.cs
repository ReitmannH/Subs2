...
namespace Subs.Data
{
    ...
    public static class LedgerData
    {
        ...
        public static void LoadInvoiceBatch(int pTransactionId, ref LedgerDoc2 pLedgerDoc)
        {
            SqlConnection lConnection = new SqlConnection();
            ...
            try
            {
                ...
                SqlCommandBuilder.DeriveParameters(Command);
                Command.Parameters["@Type"].Value = "FromTransactionId";
                Command.Parameters["@Id"].Value = pTransactionId;
        ...
        public static string GetReportInfo(int CustomerId, ref LedgerDoc2 pLedgerDoc)
        {

            string lStage = "Enter try";

            try 
            { 
                DeliveryData lDeliveryData = new DeliveryData();
                DeliveryDoc lDeliveryDoc = new DeliveryDoc();
                CustomerData3 lCustomerData;
                DeliveryAddressData2 lDeliveryAddressData;
          
                lCustomerData = new CustomerData3(CustomerId);
                pLedgerDoc.ReportInfo.Clear();

                lStage = "LoadLabel";

                // Load the label data for the payer
                if (!lDeliveryData.LoadLabelByCustomer(CustomerId, ref lDeliveryDoc))
                {
                    throw new Exception("Error in LoadLabelByCustomer");
                }

                lStage = "Format label";
                // Format the label
                if (!lDeliveryData.DefragSingleLabel(ref lDeliveryDoc))
                {
                    throw new Exception("Error in DefragSingleLabel");
                }

                lStage = "Copy report data";

                // Copy the report data into Reportinfo

                Data.LedgerDoc2.ReportInfoRow NewRow = pLedgerDoc.ReportInfo.NewReportInfoRow();
                NewRow.PayerId = CustomerId;

                // Postal address Pick up only column 1

                if (!lDeliveryDoc.Accross[0].IsL1C1Null())
                { NewRow.Line1 = lDeliveryDoc.Accross[0].L1C1; }

                if (!lDeliveryDoc.Accross[0].IsL2C1Null())
                { NewRow.Line2 = lDeliveryDoc.Accross[0].L2C1; }

                if (!lDeliveryDoc.Accross[0].IsL3C1Null())
                { NewRow.Line3 = lDeliveryDoc.Accross[0].L3C1; }

                if (!lDeliveryDoc.Accross[0].IsL4C1Null())
                { NewRow.Line4 = lDeliveryDoc.Accross[0].L4C1; }

                if (!lDeliveryDoc.Accross[0].IsL5C1Null())
                { NewRow.Line5 = lDeliveryDoc.Accross[0].L5C1; }

                if (!lDeliveryDoc.Accross[0].IsL6C1Null())
                { NewRow.Line6 = lDeliveryDoc.Accross[0].L6C1; }

                if (!lDeliveryDoc.Accross[0].IsL7C1Null())
                { NewRow.Line7 = lDeliveryDoc.Accross[0].L7C1; }

                if (!lDeliveryDoc.Accross[0].IsL8C1Null())
                { NewRow.Line8 = lDeliveryDoc.Accross[0].L8C1; }

                if (!lDeliveryDoc.Accross[0].IsL9C1Null())
                { NewRow.Line9 = lDeliveryDoc.Accross[0].L9C1; }

                lStage = "Physical Address";

                // Physical address

                lDeliveryAddressData = new DeliveryAddressData2((int)lCustomerData.PhysicalAddressId);
                ...
                NewRow.PAddress1 = lDeliveryAddressData.PAddress1;
                NewRow.PAddress2 = lDeliveryAddressData.PAddress2;
                NewRow.PAddress3 = lDeliveryAddressData.PAddress3;
                NewRow.PAddress4 = lDeliveryAddressData.PAddress4;
                NewRow.PAddress5 = lDeliveryAddressData.PAddress5;

                lStage = "Remaining bits";

                // Get the remaining bits and pieces

                NewRow.PPhoneNumber = lCustomerData.PhoneNumber;
                NewRow.PEmail = lCustomerData.EmailAddress;
                NewRow.PVATRegistration = lCustomerData.VATRegistration;
                NewRow.GovernmentSupplierNumber = lCustomerData.VendorNumber;
                NewRow.CompanyRegistrationNumber = lCustomerData.CompanyRegistrationNumber;

                pLedgerDoc.ReportInfo.AddReportInfoRow(NewRow);
                pLedgerDoc.ReportInfo.AcceptChanges();
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, "static.LedgerData", "GetReportInfo", "CustomerId = " + CustomerId.ToString() + " Stage= " + lStage );
                    CurrentException = CurrentException.InnerException;
                } while (CurrentException != null);

                return "Error in ReportInfo " + ex.Message;
            }
        }
        ...
        public static bool InitialiseSubscription(ref SqlTransaction myTransaction, int SubscriptionId, decimal pTotalCost)
        {
            try
            {
                SqlCommand Command = new SqlCommand();
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, "Static LedgerData", "InitialiseSubscription", "");
                    CurrentException = CurrentException.InnerException;
                } while (CurrentException != null);
